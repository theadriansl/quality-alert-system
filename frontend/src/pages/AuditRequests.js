    ...
    return (
      <div style={styles.container}>
        <div style={{ textAlign: 'center', padding: '48px', color: t.textMuted }}>
          Cargando solicitudes de auditoría...
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div>
          <h1 style={styles.title}> Solicitudes de Auditoría</h1>
          <p style={{ fontSize: '14px', color: t.textMuted, marginTop: '4px' }}>
            Haz clic en una solicitud para abrir el checklist y validar
          </p>
        </div>
      ...
      <div style={styles.tabs}>
        ...
        <button
          style={{
            ...styles.tab,
            backgroundColor: activeTab === 'ECR' ? t.accent : 'transparent',
            color: activeTab === 'ECR' ? 'white' : t.textMuted
          }}
      ...
      {filteredRequests.length === 0 ? (
        <div style={{ ...styles.card, textAlign: 'center', padding: '48px', cursor: 'default' }}>
          <p style={{ fontSize: '18px', marginBottom: '12px', color: t.textMuted }}>
            No hay solicitudes de auditoría {activeTab}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: t.border }}>|</span>
                  <span style={{ fontSize: '13px', color: t.textMuted }}> {request.daysOpen || 0} días</span>
                </div>
                {hasOverdue && (
                  <span style={{
                    ...styles.badge,
                    ...
                    animation: 'pulse 2s infinite'
                     ...
                     {request.overdueItems} VENCIDO(S)
                  </span>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: '24px', marginBottom: '16px', flexWrap: 'wrap', fontSize: '13px' }}>
                <div>
                  <span style={{ color: t.textMuted }}>Severidad: </span>
                  <span style={{
                    padding: '2px 8px',
                    borderRadius: '4px',
                    fontWeight: '600',
                ...
                <div>
                  <span style={{ color: t.textMuted }}>Issue: </span>
                  <span style={{ fontWeight: '500', color: t.text }}>
                    {request.issueDate ? new Date(request.issueDate).toLocaleDateString('es-MX') : '-'}
                  </span>
                </div>
                {request.clientName && (
                  <div>
                    <span style={{ color: t.textMuted }}>Cliente: </span>
                    <span style={{ fontWeight: '500', color: t.text }}>{request.clientName}</span>
                  </div>
                )}
                {request.projectNumber && (
                  <div>
                    <span style={{ color: t.textMuted }}>Proyecto: </span>
              ...
              <div style={{ marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px', fontSize: '12px' }}>
                  <span style={{ color: t.textMuted }}>Progreso de Auditoría</span>
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  {request.totalRejections > 0 && (
                    <span style={{ ...styles.badge, backgroundColor: '#fae8ff', color: '#a21caf' }}>
                       {request.totalRejections} Rechazo{request.totalRejections > 1 ? 's' : ''}
                    </span>
                  )}
                </div>
                ...
                <div style={{ fontSize: '13px' }}>
                  <span style={{ color: t.textMuted }}>Auditores: </span>
                  <span style={{
                    fontWeight: '600',
                    color: respondedCount === assignedCount && assignedCount > 0 ? t.success : t.warning
                  }}>
                    {respondedCount}/{assignedCount} respondieron
                  </span>
                </div>
                ...
                {request.nextDueDate && (
                  <>
                    <span style={{ color: t.border }}>|</span>
                    <div style={{ fontSize: '13px' }}>
                      <span style={{ color: t.textMuted }}>Próx. vencimiento: </span>
                      <span style={{
                        fontWeight: '600',
                        color: new Date(request.nextDueDate) < new Date() ? t.error : t.text
                      }}>
                        {new Date(request.nextDueDate).toLocaleDateString('es-MX')}
                      </span>
                    </div>
                  </>
                )}
        ...
        <div style={styles.modal} onClick={() => setShowChecklistModal(false)}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: '14px', color: t.textMuted }}>
                     {reportInfo?.daysOpen || 0} días
                  </span>
                </div>
                <div style={{ display: 'flex', gap: '20px', marginTop: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
                  <div>
                    <span style={{ fontSize: '12px', color: t.textMuted }}>Severidad:</span>
                    <span style={{
                      marginLeft: '8px',
                      padding: '4px 10px',
                      borderRadius: '6px',
                  ...
                  <div>
                    <span style={{ fontSize: '12px', color: t.textMuted }}>Fecha Issue:</span>
                    <span style={{ marginLeft: '8px', fontSize: '13px', fontWeight: '500', color: t.text }}>
                      {reportInfo?.issueDate ? new Date(reportInfo.issueDate).toLocaleDateString('es-MX') : '-'}
                    </span>
                  </div>
                  {reportInfo?.partsInfo?.[0]?.clientName && (
                    <div>
                      <span style={{ fontSize: '12px', color: t.textMuted }}>Cliente:</span>
                      <span style={{ marginLeft: '8px', fontSize: '13px', fontWeight: '500', color: t.text }}>
                        {reportInfo.partsInfo[0].clientName}
                      </span>
                    </div>
                  ...
                  {reportInfo?.partsInfo?.[0]?.projectNumber && (
                    <div>
                      <span style={{ fontSize: '12px', color: t.textMuted }}>Proyecto:</span>
                      <span style={{ marginLeft: '8px', fontSize: '13px', fontWeight: '500', color: t.text }}>
                        {reportInfo.partsInfo[0].projectNumber}
                      </span>
                    </div>
            ...
            <div style={styles.modalBody}>
              {loadingChecklist ? (
                <div style={{ textAlign: 'center', padding: '40px', color: t.textMuted }}>Cargando checklist...</div>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={styles.table}>
                    <thead>
                      <tr>
                        <th style={{ ...styles.th, width: '100px' }}>Categoría</th>
                        <th style={{ ...styles.th, width: '200px' }}>Check Item</th>
                        <th style={{ ...styles.th, width: '150px', backgroundColor: '#eff6ff' }}>Notas del Líder</th>
                        <th style={{ ...styles.th, width: '90px' }}>Fecha Límite</th>
                        <th style={{ ...styles.th, width: '200px' }}>Comentarios Auditor</th>
                        <th style={{ ...styles.th, width: '100px' }}>Juicio</th>
                        <th style={{ ...styles.th, width: '80px' }}>Completado</th>
                        <th style={{ ...styles.th, width: '120px' }}>Validado por</th>
                        <th style={{ ...styles.th, width: '70px', backgroundColor: '#fef9c3' }}>Ronda</th>
                        <th style={{ ...styles.th, width: '70px' }}>Hist.</th>
                      </tr>
                    </thead>
                    <tbody>
                          ...
                          <tr key={item.id} style={{
                            ...
                            <td style={styles.td}>
                              <div style={{
                                ...
                                {item.checkItem || <span style={{ color: t.textDim }}>Sin especificar</span>}
                              </div>
                            </td>
                            ...
                            <td style={styles.td}>
                              {isSelfVerified ? (
                                <div style={{ fontSize: '12px', color: t.textMuted, fontStyle: 'italic' }}>
                                  {item.auditorComments || 'Auto-verificado por líder'}
                                </div>
                              ) : (
                                <textarea
                                  ...
                                  placeholder="Comentarios / hallazgos..."
                                />
                              )}
                            </td>
                            <td style={styles.td}>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        <span style={{ fontWeight: '700', fontSize: '16px', color: t.success }}>
                          Ronda Actual: {historyData.currentRound.auditRound || 1}
                        </span>
                      ...
                      {historyData.currentRound.auditorComments && (
                        <div style={{ marginTop: '8px', fontSize: '14px', color: t.text }}>
                          <strong>Comentarios:</strong> {historyData.currentRound.auditorComments}
                        </div>
                      )}
                        ...
                        <div key={idx} style={{
                          ...
                          {round.closureNotes && (
                            <div style={{ fontSize: '13px', color: t.success, marginBottom: '6px' }}>
                              <strong>Resolución:</strong> {round.closureNotes}
                            </div>
                          )}
                          <div style={{ fontSize: '12px', color: t.textDim }}>
                            {round.auditorName && <span> {round.auditorName}</span>}
                            {round.verificationDate && (
                              <span> - {new Date(round.verificationDate).toLocaleDateString('es-MX')}</span>
                            )}
                            {round.closedAt && (
                              <span> - Cerrado: {new Date(round.closedAt).toLocaleDateString('es-MX')}</span>
                            )}
                          </div>
                        </div>
                      ))}
